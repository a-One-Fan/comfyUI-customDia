...
class Dia:
    ...
    def _prepare_audio_prompt(self, audio_prompt: torch.Tensor | None) -> tuple[torch.Tensor, int]:
        ...
        if audio_prompt is not None:
            ...
            if audio_prompt.shape[1] == 2:  # stéréo
                audio_prompt = torch.mean(audio_prompt, dim=1, keepdim=True)  # Convertir en mono (1, T)
            for i in range(0,20):
                print(f"prefill shape: {prefill.shape}")
                print(f"audio_prompt shape: {audio_prompt.shape}")
